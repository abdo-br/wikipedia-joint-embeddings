...
def annotate(article):
    ...
    annotations = pd.DataFrame(columns=['Article', 'Mention', 'Entity', 'EntityID', 'Offset', 'Sentence', 'The_Sentence'], dtype='unicode', index=None)
    ...
    for entity in article_entities:
        for pair in re.finditer(nlp.get_entity_pattern(entity), regex_input):
            ...
            if nlp.invalid_entity(entity):
                continue
            annotations.loc[len(annotations.index)] = [article.page_name, mention, entity, nlp.get_entity_id(entity), pair.start() , -1, None]
    ...
    # improve sentence tokenization for the article
    sentence_tokenzier = nlp.trained_punkt_tokenizer(article_body)

    # map offsets to sentences
    sentences_spans = []
    tokenized_sents = nlp.get_sentences(article_body, sentence_tokenzier)
...
gold_standard = pd.DataFrame(columns=['Article', 'Mention', 'Entity', 'EntityID', 'Offset', 'Sentence', 'The_Sentence'], dtype='unicode', index=None)
