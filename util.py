...
def get_entities(chunks=False): # we need to clean redirects
    ...
    if chunks:
        ...
        for chunk in pd.read_csv(settings.PATH_ENTITIES_ARTICLES, sep=' ',
                               header=None, index_col=False, dtype='unicode', chunksize=5000000):
        ...
        entities['entity'] = entities['entity'].apply(lambda x: urllib.parse.unquote
                                                      (str(x), encoding='utf-8', errors='replace'))
